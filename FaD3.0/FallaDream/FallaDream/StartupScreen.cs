...
namespace FallaDream
{
    class StartupScreen
    {
        ...
        public  StartupScreen(SpriteBatch _s,GraphicsDevice _g)
        {
            s = _s;
            g = _g;
            Stream s0 = File.OpenRead("Content/images/FallaDream.png");
            imgBGI = Texture2D.FromStream(g, s0);
            Stream s1 = File.OpenRead("Content/images/storymode0.png");
            imgBtn0 = Texture2D.FromStream(g, s1);
            Stream s2 = File.OpenRead("Content/images/challengemode0.png");
            imgBtn1 = Texture2D.FromStream(g, s2);
        }
       ...
       public  void MouseHoverChk(int x,int y)
        { 
               if (x >= posBtn0.X && x < posBtn0.X + 250)
               {
                if (y >= posBtn0.Y && y < posBtn0.Y + 45)
                {
                    Stream s1 = File.OpenRead("Content/images/storymode1.png");
                    imgBtn0 = Texture2D.FromStream(g, s1);
                }
                else
                {
            ...
            if (x >= posBtn1.X && x < posBtn1.X + 250)
            {
                if (y >= posBtn1.Y && y < posBtn1.Y + 45)
                {
                    Stream s1 = File.OpenRead("Content/images/challengemode1.png");
                    imgBtn1 = Texture2D.FromStream(g, s1);
                }
                else
                {
                    Stream s1 = File.OpenRead("Content/images/challengemode0.png");
                    imgBtn1 = Texture2D.FromStream(g, s1);
                }
